...
def Get():
	...
	m3u  = 'http://allthingziptv.ddns.net:80/get.php?username='+username+'&password='+password+'&type=m3u_plus&output=ts'
	epg  = 'allthingziptv.ddns.net:80/xmltv.php?username='+username+'&password='+password
	auth = 'allthingziptv.ddns.net:80/enigma2.php?username='+username+'&password='+password+'&type=get_vod_categories'
	...
	if not auth=="":
		...
		xbmcgui.Dialog().ok('allthingz','[COLOR blue]M3U URL: [/COLOR]%s'%shortm3u,'','[COLOR blue]EPG URL: [/COLOR]%s'%shortepg)
	else:
		return