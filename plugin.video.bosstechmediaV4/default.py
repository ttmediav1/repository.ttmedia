...
#############Defined Strings#############
addon_id     = 'plugin.video.bosstechmediaV4'
...
movie_url    =  '%s/movie/%s/%s/'%(host,username,password)
Series_url   = '%s:%s/enigma2.php?username=%s&password=%s&type=get_series_categories'%(host,port,username,password)


Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV4/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV4/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to bosstechmediaV4"
			line3 = ('[COLOR cornsilk]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('bosstechmediaV4', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
def listcatchup():
	...
	for a in all:
		if '"tv_archive":1' in a:
			...
			tools.addDir(name.replace('ENT:','[COLOR red]ENT:[/COLOR]').replace('DOC:','[COLOR red]DOC:[/COLOR]').replace('MOV:','[COLOR red]MOV:[/COLOR]').replace('SSS:','[COLOR red]SSS[/COLOR]').replace('BTS:','[COLOR red]BTS:[/COLOR]').replace('TEST','[COLOR red]TEST[/COLOR]').replace('Install','[COLOR red]Install[/COLOR]').replace('24/7','[COLOR red]24/7[/COLOR]').replace('INT:','[COLOR red]INT:[/COLOR]').replace('DE:','[COLOR red]DE:[/COLOR]').replace('FR:','[COLOR red]FR:[/COLOR]').replace('PL:','[COLOR red]PL:[/COLOR]').replace('AR:','[COLOR red]AR:[/COLOR]').replace('LIVE:','[COLOR red]LIVE:[/COLOR]').replace('ES:','[COLOR red]ES:[/COLOR]').replace('IN:','[COLOR red]IN:[/COLOR]').replace('PK:','[COLOR red]PK:[/COLOR]'),'url',13,thumb,fanart,id)
			

def tvarchive(name,description):
    name = str(name.replace('[COLOR red]ENT:[/COLOR]','ENT:').replace('[COLOR red]DOC:[/COLOR]','DOC:').replace('[COLOR red]MOV:[/COLOR]','MOV').replace('[COLOR red]SSSS[/COLOR]','SSS:').replace('[COLOR red]BTS:[/COLOR]','BTS:').replace('[COLOR red]INT:[/COLOR]','INT:').replace('[COLOR red]DE:[/COLOR]','DE:').replace('[COLOR red]FR:[/COLOR]','FR:').replace('[COLOR red]PL:[/COLOR]','PL:').replace('[COLOR red]AR:[/COLOR]','AR:').replace('[COLOR red]LIVE:[/COLOR]','LIVE:').replace('[COLOR red]ES:[/COLOR]','ES:').replace('[COLOR red]IN:[/COLOR]','IN:').replace('[COLOR red]PK:[/COLOR]','PK'))
    filename = open(Guide)
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("bosstechmediaV4", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search bosstechmediaV4:')
	if search=="":
		return
	else:
		return search
...
def search():
	...
	xbmc.log(str(text))
	open = tools.OPEN_URL(panel_api)
	all_chans = tools.regex_get_all(open,'{','}')
	for a in all_chans:
		...
		category_id = tools.regex_from_to(a,'category_id":"','"').replace('\/','/')
		series_name = tools.regex_from_to(a,'category_name":"','"').replace('\/','/')
                if text.lower() in series_name.lower():
                        series_url = "http://www.ipcctv.org/enigma2.php?username=%s&password=%s&type=get_series&cat_id=%s" % (username,password,category_id)
			tools.addDir(series_name,series_url,21,thumb,fanart,'')
		elif text in name.lower():
                        if stream_type=="movie":
                                tools.addDir(name,movie_url+url+'.mkv',4,thumb,fanart,'')
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('bosstechmediaV4', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('bosstechmediaV4', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV4', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('bosstechmediaV4', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV4', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.bosstechmediaV4/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('bosstechmediaV4', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def accountinfo():
	...
	try:
		...
		for day,month,year in expreg:
			...
			tools.addDir('[COLOR red]Local IP Address:[/COLOR] '+ip,'','',icon,fanart,'')
			tools.addDir('[COLOR red]External IP Address:[/COLOR] '+extip,'','',icon,fanart,'')
	except:pass
		
	
def correctPVR():

	addon = xbmcaddon.Addon('plugin.video.bosstechmediaV4')
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('bosstechmediaV4','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
