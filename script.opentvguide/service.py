...
if __name__ == '__main__':
    ...
    try:
        ADDON = xbmcaddon.Addon('script.opentvguide')
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.opentvguide)")
        
        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.opentvguide] Background service started...", xbmc.LOGDEBUG)
            Service()
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[script.opentvguide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[script.opentvguide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[script.opentvguide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
