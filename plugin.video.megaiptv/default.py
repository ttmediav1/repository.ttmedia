...
Guide = xbmc.translatePath(os.path.join('special://home/addons/addons/'+user.id+'/resources', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/addons/'+user.id+'/resources/', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line3 = ('[B][COLOR gold]%s[/COLOR][/B]'%usern)
...
def search():
	...
	text = searchdialog()
	if not text:
		xbmc.executebuiltin("XBMC.Notification([B][COLOR red][B]Search is Empty[/B][/COLOR][/B],Aborting search,4000,"+icon+")")
		return
	xbmc.log(str(text))
