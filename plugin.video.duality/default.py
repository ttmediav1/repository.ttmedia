...
#################################
service_name =  '[B][COLORdodgerblue]Duality[/B][/COLOR]'
...
speedtest	 = xbmc.translatePath(os.path.join('special://home/addons/' + user.id , 'speedtest.png'))
...
live_url2     = '%s:%s/enigma2.php?username=%s&password=%s&type=get_live_categories'%(user.host1,user.port1,username,password)
...
def start():
	if username=="":
		...
		if not auth=="":
			...
			tools.addDir('Back up TV','live',21,icon,fanart,'')
...
def livecategory(url):
	...
	for a in all_cats:
		...
		if xbmcaddon.Addon().getSetting('hidexxx')=='true':
			tools.addDir('%s'%name,url1,2,icon,fanart,thumb,'')
		else:
			if not 'XXX |' in name:
				tools.addDir('%s'%name,url1,2,icon,fanart,'')
				
				
def livecategory2(url):
	
	open = tools.OPEN_URL(live_url2)
	all_cats = tools.regex_get_all(open,'<channel>','</channel>')
...
def addonsettings(url,description):
	...
	if   url =="CC":
		...
		if 'ON' in description:
			xbmcaddon.Addon().setSetting('hideARABIC','true')
			xbmc.executebuiltin('Container.Refresh')
		else:
			xbmcaddon.Addon().setSetting('hideARABIC','true')
			xbmc.executebuiltin('Container.Refresh')
...
def extras():
	tools.addDir('Run a Speed Test','ST',10,speedtest,fanart,'')
