...
#############Defined Strings#############
addon_id     = 'plugin.video.bosstechmediaV2'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV2/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV2/resources/catchup', 'g'))
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("bosstechmediaV2", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search bosstechmediaV2:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('bosstechmediaV2', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('bosstechmediaV2', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV2', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('bosstechmediaV2', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV2', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.bosstechmediaV2/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('bosstechmediaV2', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.bosstechmediaV2')
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('bosstechmediaV2','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
