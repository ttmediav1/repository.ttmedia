...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to bosstechmediaV3" 
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("bosstechmediaV3", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search bosstechmediaV3:')
	if search=="":
		return
	else:
		return search
...
def asettings():
	choice = xbmcgui.Dialog().yesno('bosstechmediaV3', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.bosstechmediaV3')
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('bosstechmediaV3','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
