...
#############Defined Strings#############
addon_id     = 'plugin.video.bosstechmediaV6'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV6/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bosstechmediaV6/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		auth = tools.OPEN_URL(auth)
		if not auth=="":
			tools.addDir('[COLORdeepskyblue]Account Information[/COLOR]','url',6,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Live Tv[/COLOR]','live',1,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Catchup TV[/COLOR]','url',12,icon,fanart,'')
			if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)') or xbmc.getCondVisibility('System.HasAddon(script.ivueguide)'):
				tools.addDir('[COLORdeepskyblue]TV Guide[/COLOR]','pvr',7,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]VOD[/COLOR]','vod',3,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Series[/COLOR]','Series',20,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Search[/COLOR]','url',5,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Settings[/COLOR]','url',8,icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Extras[/COLOR]','url',16,icon,fanart,'')
def home():
...
##########################################
def catchup():
    loginurl   = "http://ll-tv.xyz:24561/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
        ...
        print e.getcode()
        dialog.ok("[COLORdeepskyblue]Expired Account[/COLOR]",'[COLORdeepskyblue]You cannot use this service with an expired account[/COLOR]',' ','[COLORdeepskyblue]Please check your account information[/COLOR]')
        sys.exit(1)
...
def listcatchup():
	...
	for a in all:
		if '"tv_archive":1' in a:
			...
			tools.addDir(name.replace('ENT:','[COLORdeepskyblue]ENT:[/COLOR]').replace('DOC:','[COLORdeepskyblue]DOC:[/COLOR]').replace('MOV:','[COLORdeepskyblue]MOV:[/COLOR]').replace('SSS:','[COLORdeepskyblue]SSS[/COLOR]').replace('BTS:','[COLORdeepskyblue]BTS:[/COLOR]').replace('TEST','[COLORdeepskyblue]TEST[/COLOR]').replace('Install','[COLORdeepskyblue]Install[/COLOR]').replace('24/7','[COLORdeepskyblue]24/7[/COLOR]').replace('INT:','[COLORdeepskyblue]INT:[/COLOR]').replace('DE:','[COLORdeepskyblue]DE:[/COLOR]').replace('FR:','[COLORdeepskyblue]FR:[/COLOR]').replace('PL:','[COLORdeepskyblue]PL:[/COLOR]').replace('AR:','[COLORdeepskyblue]AR:[/COLOR]').replace('LIVE:','[COLORdeepskyblue]LIVE:[/COLOR]').replace('ES:','[COLORdeepskyblue]ES:[/COLOR]').replace('IN:','[COLORdeepskyblue]IN:[/COLOR]').replace('PK:','[COLORdeepskyblue]PK:[/COLOR]'),'url',13,thumb,fanart,id)
			

def tvarchive(name,description):
    name = str(name.replace('[COLORdeepskyblue]ENT:[/COLOR]','ENT:').replace('[COLORdeepskyblue]DOC:[/COLOR]','DOC:').replace('[COLORdeepskyblue]MOV:[/COLOR]','MOV').replace('[COLORdeepskyblue]SSSS[/COLOR]','SSS:').replace('[COLORdeepskyblue]BTS:[/COLOR]','BTS:').replace('[COLORdeepskyblue]INT:[/COLOR]','INT:').replace('[COLORdeepskyblue]DE:[/COLOR]','DE:').replace('[COLORdeepskyblue]FR:[/COLOR]','FR:').replace('[COLORdeepskyblue]PL:[/COLOR]','PL:').replace('[COLORdeepskyblue]AR:[/COLOR]','AR:').replace('[COLORdeepskyblue]LIVE:[/COLOR]','LIVE:').replace('[COLORdeepskyblue]ES:[/COLOR]','ES:').replace('[COLORdeepskyblue]IN:[/COLOR]','IN:').replace('[COLORdeepskyblue]PK:[/COLOR]','PK'))
    filename = open(Guide)
        ...
        if name in programme.attrib.get('channel'):
            ...
            if head > date:
                if head < now:
                    ...
                    head2 = '[COLORdeepskyblue]%s - [/COLOR]'%head2 
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        try: 
            ...
            mbs = '[COLORdeepskyblue]%.02f MB of less than 5MB[/COLOR]' % (currently_downloaded)
            e = '[COLORdeepskyblue]Speed:  %.02f Mb/s ' % mbps_speed  + '[/COLOR]'
        ...
        if dp.iscanceled():
            ...
            dialog.ok("bosstechmediaV6", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search bosstechmediaV6:')
	if search=="":
		return
	else:
		return search
...
def search():
	...
	text = searchdialog()
	if not text:
		xbmc.executebuiltin("XBMC.Notification([COLORdeepskyblue][B]Search is Empty[/B][/COLOR],Aborting search,4000,"+icon+")")
		return
	xbmc.log(str(text))
	open = tools.OPEN_URL(panel_api)
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('bosstechmediaV6', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('bosstechmediaV6', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV6', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('bosstechmediaV6', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('bosstechmediaV6', 'PVR & iVue Integration Complete')
	elif url =="ST":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('bosstechmediaV6', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def accountinfo():
	...
	try:
		...
		for day,month,year in expreg:
			...
			tools.addDir('[COLORdeepskyblue]Username :[/COLOR] '+username,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Password :[/COLOR] '+password,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Expiry Date:[/COLOR] '+expiry,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Account Status :[/COLOR] %s'%status,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Current Connections:[/COLOR] '+ active,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Allowed Connections:[/COLOR] '+connects,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]Local IP Address:[/COLOR] '+ip,'','',icon,fanart,'')
			tools.addDir('[COLORdeepskyblue]External IP Address:[/COLOR] '+extip,'','',icon,fanart,'')
	except:pass
		
	
def correctPVR():

	addon = xbmcaddon.Addon('plugin.video.bosstechmediaV6')
	...
	loginurl   = "http://ll-tv.xyz:24561/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://ll-tv.xyz:24561/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('bosstechmediaV6','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
