...
mysettings = xbmcaddon.Addon(id = 'plugin.video.ttmediatv')
...
online_m3u1 = mysettings.getSetting('online_m3u1')
online_m3u2 = mysettings.getSetting('online_m3u2')
online_m3u3 = mysettings.getSetting('online_m3u3')
online_m3u4 = mysettings.getSetting('online_m3u4')
...
def main():
	...
	if len(online_m3u) > 0:	
		add_dir('[COLORdodgerblue][B]TTMedia TV[/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(online_m3u1) > 0:	
		add_dir('[COLOR yellow][B][/B][/COLOR]', u_tube, 6, icon, fanart)
	if len(online_m3u2) > 0:	
		add_dir('[COLOR yellow][B][/B][/COLOR]', u_tube, 7, icon, fanart)
	if len(online_m3u3) > 0:	
		add_dir('[COLOR yellow][B][/B][/COLOR]', u_tube, 8, icon, fanart)
	if len(online_m3u4) > 0:	
		add_dir('[COLOR yellow][B][/B][/COLOR]', u_tube, 9, icon, fanart)	
	if len(local_m3u) > 0:	
...
def search(): 	
	try:
		...
		if len(online_m3u1) > 0:		
			content = make_request(online_m3u1)
			match = re.compile(m3u_regex).findall(content)
			for thumb, name, url in match:
				if re.search(searchText, removeAccents(name.replace('Đ', 'D')), re.IGNORECASE):
					m3u_playlist(name, url, thumb)				
...
def m3u_online2():		
	content = make_request(online_m3u2)
	match = re.compile(m3u_regex).findall(content)
	for thumb, name, url in match:
		try:
			m3u_playlist(name, url, thumb)
...
def m3u_online3():		
	content = make_request(online_m3u3)
	match = re.compile(m3u_regex).findall(content)
	for thumb, name, url in match:
		try:
			m3u_playlist(name, url, thumb)
...
def m3u_online4():		
	content = make_request(online_m3u4)
	match = re.compile(m3u_regex).findall(content)
	for thumb, name, url in match:
		try:
			m3u_playlist(name, url, thumb)
